	...
	static void bfs() {
		...
		while(!queue.isEmpty()) {
			...
			for(int i = 0; i < 4; i++) {
				...
				if(now.z > 0 &&!visit[nx][ny][now.z - 1] && arr[nx][ny] == 1) {
					queue.offer(new B(nx, ny, now.z - 1, now.time + 1));
					visit[nx][ny][now.z - 1] = true;
				}else if(!visit[nx][ny][now.z] && arr[nx][ny] == 0) {
					queue.offer(new B(nx, ny, now.z, now.time + 1));
